...
case "$MPI_IMPL" in
openmpi-3.1.0)
   if [ ! -d "${HOME}/local-mpi/${MPI_IMPL}/bin" ]
   then
      ...
      cd openmpi-3.1.0
      ./configure --prefix=${HOME}/local-mpi/${MPI_IMPL} --disable-wrapper-rpath --disable-wrapper-runpath && make -j 2 && make install && make clean
      cd ../../
      exit 0
   else
